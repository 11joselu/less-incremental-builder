...
class Compile {
          ...
          .pipe(
            through(async function(file, enc, done) {
              ...
              if (removedImports.length) {
                const _removedImports = removedImports.filter(delImport => {
                  return self.graph.shouldBeUnWatched(delImport, filePath);
                });

                _removedImports.forEach(self.unwatchFile.bind(self));
              }
  ...
  unwatchFile(filePath) {
    ...
    if (index !== -1) {
      ...
      this._unwatchFileImports(filePath);
    }
  }

  _unwatchFileImports(filePath) {
    this.graph
      .getImportsFromFile(filePath)
      .filter(impFilePath => {
        return this.graph.shouldBeUnWatched(impFilePath, filePath);
      })
      .forEach(this.unwatchFile.bind(this));
  }
}
